...
\hitsetup{
  ...
  ctitlecover={中文语义依存图分析\\技术及应用研究},%放在封面中使用，自由断行
  ctitle={中文语义依存图分析技术及应用研究},%放在原创性声明中使用
  ...
  % 英文信息
  %=========
  etitle={Chinese Semantic Dependency Graph Parsing and Application},
...
2.\textbf{语义依存图分析器鲁棒性增强方法}。
尽管基于深度学习的依存分析器在现有的语料库上获得了较好的性能，但研究发现，只要对输入进行略微修改，其性能就会显著下降。
针对这一问题，本文设计了对抗样本攻击算法用于生成使分析器产生误判的对抗样本，并利用此算法对基于深度学习的依存分析器的鲁棒性进行深入探究。
本文进一步提出模型融合和对抗样本训练方法，有效提升了基于深度学习的语义依存图分析器的鲁棒性。
...
总的来说，本文从中文语义依存图分析方法本身开始，深入探究了语义依存图分析器鲁棒性增强方法和跨语言的语义依存图分析技术，最终将语义依存图中的信息融入预训练模型并将其应用于其他自然语言处理任务，从而显著提升了他们的性能。
